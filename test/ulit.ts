...
describe("Part", () => {
  it("should have {value, path, disposer, target, isSVG, prop}", () => {
    const fragment = document.createDocumentFragment();
    const target = document.createElement("div");
    fragment.appendChild(target);
    const test1 = new Part(fragment, [0]);
    ...
    expect(test1.prop !== undefined).to.equal(true);
  });
  ...
  it("update should replace initial comment node", () => {
    ...
    const part = new Part(fragment, [0], false);
  ...
  it("update should correctly handle attributes", done => {
    ...
    const part = new Part(fragment, [0], false, "id");
...
describe("DomTarget", () => {
  ...
  it("should properly identify first and last", () => {
    const fragment = document.createDocumentFragment();
    const startPart = document.createComment("{{}}");
    const endPart = document.createComment("{{}}");
    fragment.appendChild(startPart);
    fragment.appendChild(endPart);
    const t1 = new DomTarget();
    t1.start = new Part(fragment, [0], false);
    t1.end = new Part(fragment, [1], false);
    expect(t1.first()).to.equal(startPart);
    expect(t1.last()).to.equal(endPart);
    t1.start = startPart;
    t1.end = startPart;
    expect(t1.first()).to.equal(startPart);
    expect(t1.last()).to.equal(startPart);
  });
...
describe("render", () => {
  ...
  it("attribute directives should work as expected", () => {
    const template = (str: string) => html`<div id=${directive(part => (part as Part).update(str))}>test</div>`;
    render(template("test"), target);
