...
test("tagged template literal should handle nested template", async t => {
  ...
  const template1 = await html`<div>${await html`<div id="test">test</div>`}</div>`;
  template1.update();
...
test("tagged template literal should allow an expression which changes types between renders", async t => {
  const str = "test";
  const div = document.createElement("div");
  div.id = "test";
  const template = await html`<div>${str}</div>`;
  template.update();
  t.is(template.fragment.content.firstChild.firstChild.nodeValue, "test");
  template.update([div]);
  t.is(template.fragment.content.firstChild.firstChild.id, "test");
